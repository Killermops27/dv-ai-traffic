...
namespace AITraffic.Fleet
{
    ...
    {
        /// <summary>
        /// Spawns an AI train consist of the specified type on the given track.
        /// </summary>
        ...
        public static AIEngineer SpawnAITrain(RailTrack track, ConsistType consistType, double startSpan = 15.0, bool flipTrainConsist = false, System.Random rng = null)
        {
            ...
            List<TrainCarLivery> liveries = ConsistDefinitions.GetLiveries(consistType, rng);
            if (liveries == null || liveries.Count == 0)
            {
                if (Main.ModEntry != null && Main.ModEntry.Logger != null)
                    Main.ModEntry.Logger.Error(string.Format("TrainSpawner.SpawnAITrain failed: No liveries generated for consist type {0}.", consistType));
                return null;
            }

            return SpawnAITrain(track, liveries, startSpan, flipTrainConsist);
        }
        ...
        private static void CoupleAdjacentCars(TrainCar carA, TrainCar carB)
        {
            ...
            for (int i = 0; i < couplersA.Length; i++)
            {
                ...
                for (int j = 0; j < couplersB.Length; j++)
                {
                    ...
                    float dist = Vector3.Distance(cA.transform.position, cB.transform.position);
                    if (dist < bestDist)
                    {
                        bestDist = dist;
                        bestA = cA;
            ...
            if (bestA != null && bestB != null && bestDist < 5.0f)
            {
                try
                {
